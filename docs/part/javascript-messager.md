...
```js
new $.mgui.Messager('此消息无法关闭，5秒后自动关闭', {
    close: false // 禁用关闭按钮
}).show();
```
...
创建 Messager 实例方法调用形式：
 ...
 - <strong class="code text-danger">new $.mgui.Messager()</strong>
 - <strong class="code text-danger">new $.mgui.Messager(message)</strong>
 - <strong class="code text-danger">new $.mgui.Messager(options)</strong>
 - <strong class="code text-danger">new $.mgui.Messager(message, options)</strong>
...
```js
var myMessager = new $.mgui.Messager('Hello, messager!', {
    type: 'success'
});
```
...
```js
// 创建 Messager 实例
var myMessager = new $.mgui.Messager({type: 'success'});
...
```js
// 创建 Messager 实例
var myMessager1 = new $.mgui.Messager('消息一');
var myMessager2 = new $.mgui.Messager('消息二');
...
```js
// 使用jQuery对象
var msg = new $.mgui.Messager('消息内容', {placement: 'bottom'});
...
使用 `$.mgui.messager` 实例的 `show()` 方法来即时显示一个漂浮消息。

调用`$.mgui.messager.show()`或者`$.mgui.messager.show()`之后立即显示并返回新创建的`Messager`实例。可以使用返回的实例来手动执行`hide`方法隐藏此消息。

`show()`方法提供两个参数，定义于`Messager`构造函数参数定义相同。

```js
// 使用jQuery对象
var msg = $.mgui.messager.show('消息内容', {placement: 'bottom'});
...
function afterPageLoad() {
    $('#pageContent .show-messager').on('click', function() {
        var $this = $(this);
        var data = $this.data('mgui.messager');
        if(data) {
            data.show(new Date());
        }
        else {
            ...
            $this.data('mgui.messager', new $.mgui.Messager(options.content, options).show());
        }
    });
    ...
    $('#pageContent .messager-example').click(function() {
        ...
        options = $this.data();
        if(typeof options.actions === 'string') options.actions = $.parseJSON(options.actions);
        new $.mgui.Messager($this.find('.messager-content').html(), options).show();
    });
}
</script>
