...
```html
<link href="lib/imgcutter/mgui.imgcutter.min.css" rel="stylesheet">
<script src="lib/imgcutter/mgui.imgcutter.min.js"></script>
```
...
```js
// 获取 imgCutter 实例
var myImgCutter = $('#imgCutter').data('mgui.imgCutter');
...
<table class="table table-bordered">
  ...
  <tbody>
    <tr>
      <td>`ready`</td>
      <td>ready.mgui.imgcutter</td>
      <td>当初始化之后时触发此事件</td>
    </tr>
    <tr>
      <td>`change`</td>
      <td>change.mgui.imgcutter</td>
      <td>当用户更改所选择的区域时触发</td>
    </tr>
    <tr>
      <td>`before`</td>
      <td>before.mgui.imgcutter</td>
      <td>当用户确认提交所选择的区域时触发，在该事件回调函数中返回`false`可以取消此次提交</td>
    </tr>
    <tr>
      <td>`done`</td>
      <td>done.mgui.imgcutter</td>
      <td>当提交数据到服务器成功后触发</td>
    </tr>
    <tr>
      <td>`fail`</td>
      <td>fail.mgui.imgcutter</td>
      <td>当提交数据到服务器失败后触发</td>
    </tr>
    <tr>
      <td>`always`</td>
      <td>always.mgui.imgcutter</td>
      <td>当提交数据到服务器完成后触发</td>
    </tr>
  </tbody>
</table>

<script src="dist/lib/imgcutter/mgui.imgcutter.js"></script>
<link rel="stylesheet" href="dist/lib/imgcutter/mgui.imgcutter.css">
<script>
