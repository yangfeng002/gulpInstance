...
在日历上能够让用户直观了解日常安排并进行规划。mgui提供了实用简单的日历插件。
...
```html
<link href="lib/calendar/mgui.calendar.min.css" rel="stylesheet">
<script src="lib/calendar/mgui.calendar.min.js"></script>
```
...
要使用日历对象方法，需要首先获取日历对象实例。每一个日历对象示例的引用都存储在对应的jQuery对象的data中，名称为`mgui.calendar`。

```js
/* 获取日历对象实例 */
var calendar = $('#calendar').data('mgui.calendar');
```
...
<table class="table table-bordered">
  ...
  <tbody>
    <tr>
      <td>`clickTodayBtn`</td>
      <td>`"clickTodayBtn.mgui.calendar"`</td>
      <td>点击日历头部的当天按钮时发生</td>
      <td>无</td>
    </tr>
    <tr>
      <td>`clickNextBtn`</td>
      <td>`"clickNextBtn.mgui.calendar"`</td>
      <td>点击当前视图下一个日期范围按钮时发生</td>
      <td>无</td>
    </tr>
    <tr>
      <td>`clickPrevBtn`</td>
      <td>`"clickPrevBtn.mgui.calendar"`</td>
      <td>点击当前视图上一个日期范围按钮时发生</td>
      <td>无</td>
    </tr>
    <tr>
      <td>`clickEvent`</td>
      <td>`"clickEvent.mgui.calendar"`</td>
      <td>当点击一个事件时发生</td>
...
*   `event.events` 事件对象实例数组，当前所有事件</td>
    ...
    <tr>
      ...
      <td>`"beforeAddCalendars.mgui.calendar"`</td>
...
*   `event.data` 所有日历事件数据</td>
    ...
    <tr>
      <td>`addCalendars`</td>
      <td>`"addCalendars.mgui.calendar"`</td>
...
*   `event.data` 所有日历事件数据</td>
    ...
    <tr>
      ...
      <td>`"beforeAddEvent.mgui.calendar"`</td>
...
*   `event.data` 所有日历事件数据</td>
    ...
    <tr>
      ...
      <td>`"beforeRemoveEvent.mgui.calendar"`</td>
...
*   `event.data` 所有日历事件数据</td>
    ...
    <tr>
      <td>`removeEvents`</td>
      <td>`"removeEvents.mgui.calendar"`</td>
...
*   `event.data` 所有日历事件数据</td>
    ...
    <tr>
      <td>`beforeChange`</td>
      <td>`"beforeChange.mgui.calendar"`</td>
...
-   `event.to` 更改之后新的值</td>
    ...
    <tr>
      <td>`beforeDisplay`</td>
      <td>`"beforeDisplay.mgui.calendar"`</td>
...
```js
$('calendar').calendar().on("clickEvent.mgui.calendar", function(event) {
    console.log(event);
    // console.log("你点击了一个事件");
    // 处理 clickEvent 事件
    // ...
...
<script src="dist/lib/calendar/mgui.calendar.js"></script>
<link href="dist/lib/calendar/mgui.calendar.css" rel="stylesheet">
<script>
  ...
  var dtDataGenerater = function(rowsCount) {
          ...
          for (var i = 0; i < rowsCount; i++) {
              var row = {checked: Math.random() > 0.9, data: [
                  ...
                  "<a href='###'><i class='fa fa-check'></i></a> &nbsp; <a href='###' class='text-danger'><i class='fa fa-trash'></i></a> "
              ]};
              data.rows.push(row);
  ...
  $(function() {
      $('#pageContent .calendar').each(function() {
          ...
          $this.calendar({data: data, clickEvent: function(e) {
              $.mgui.messager.show('您点击了 <strong>' + e.event.title + '</strong>');
          }, beforeChange: function(e) {
              if(e.change === 'start')
              {
                  $.mgui.messager.show('起始时间更改为 ' + e.to.format('yyyy年MM月dd日 hh:mm'));
              }
          }});
      });
  });
