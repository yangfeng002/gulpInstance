...
/* 使用触发器对象直接显示 */
(new $.mgui.ModalTrigger({custom: '自定义内容'})).show();
```
...
/* 使用触发器对象直接显示 */
(new $.mgui.ModalTrigger({custom: '#myModalAlert'})).show();
```
...
/* 使用触发器对象直接显示 */
(new $.mgui.ModalTrigger({custom: function() {
    return "当对话框显示时，时间是：" + (new Date).toString();
}})).show();
```
...
/* 使用触发器对象直接显示 */
(new $.mgui.ModalTrigger({showHeader: false})).show();
```
...
/* 使用触发器对象直接显示 */
(new $.mgui.ModalTrigger({title: '新的标题'})).show();
```
...
/* 使用触发器对象直接显示 */
(new $.mgui.ModalTrigger({icon: 'heart'})).show();
```
...
在`$.mgui`对象上已默认绑定了一个对话框触发器对象，可以直接使用`方法`并传递不同的参数来随时启动对话框。

```js
$.mgui.modalTrigger.show(options);
```
...
```js
var myModalTrigger = new $.mgui.ModalTrigger(options);
myModalTrigger.show();
```
...
```js
var modalTrigger = $('#triggerButton').data('mgui.modaltrigger');
```

##### 使用预设的$.mgui对象绑定的触发器

```js
var myTrigger = $.mgui.modalTrigger;
```

##### 创建新的触发器实例

```js
var newTrigger = new $.mgui.ModalTrigger(options);
```
...
<table class="table table-bordered">
  ...
  <tbody>
    ...
    <tr>
      <td>`loaded`</td>
      <td>`loaded.mgui.modal`</td>
      <td>当远程内容加载完成后发生</td>
    </tr>
    <tr>
      <td>`broken`</td>
      <td>`broken.mgui.modal`</td>
...
```js
$('#triggerModal').on('shown.mgui.modal', function() {...});
```
