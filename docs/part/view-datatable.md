...
```html
<link href="lib/datatable/mgui.datatable.min.css" rel="stylesheet">
<script src="lib/datatable/mgui.datatable.min.js"></script>
```
...
<table class="table table-bordered">
  ...
  <tbody>
    <tr>
      <td>`afterLoad`</td>
      <td>`"afterLoad.mgui.datatable"`</td>
      <td>数据加载完毕后调用</td>
      <td>*   `event.data` 表格数据</td>
    </tr>
    <tr>
      <td>`ready`</td>
      <td>`"ready.mgui.datatable"`</td>
      <td>渲染完毕后调用，可以在此事件中为DOM绑定其他事件</td>
      <td>无</td>
    </tr>
    <tr>
      <td>`sort`</td>
      <td>`"sort.mgui.datatable"`</td>
      <td>表格重新排序之后</td>
...
*   `event.sorter.sortUp` 排序方向，`true`为升序，`false` 为降序</td>
    ...
    <tr>
      <td>`sizeChanged`</td>
      <td>`"sizeChanged.mgui.datatable"`</td>
      <td>列宽发生更改时发生</td>
...
*   `event.changes.colIndex` 数字，如果是单个列宽发生变化，则该值指示发生变化列的序号</td>
    ...
    <tr>
      <td>`checksChanged`</td>
      <td>`"checksChanged.mgui.datatable"`</td>
...
```js
$('table.datatable').datatable().on("sort.mgui.datatable", function(event) {
    console.log(event);
    // console.log("表格已重新排序！");
    // 处理 sort 事件
    // ...
...
```js
// 获取数据表格实例对象
var myDatatable = $('table.datatable').data('mgui.datatable');
...
<script src="dist/lib/datatable/mgui.datatable.js"></script>
<link href="dist/lib/datatable/mgui.datatable.css" rel="stylesheet">
<script>
function onPageLoad() {
    return false;
}
