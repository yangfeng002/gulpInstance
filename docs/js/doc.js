...
+(function(window, $) {
    ...
    if($.mgui.browser.ie && $.mgui.browser.ie < 11) {
        $.mgui.messager.danger('你正在使用 IE 较低版本访问，无法获得 MGUI 文档网站的完整体验，建议你更换浏览器再访问。', {time: 20000});
    }
    ...
    var LAST_RELOAD_ANIMATE_ID = 'lastReloadAnimate',
        ...
        MGUI_JSON = debug ? 'mgui.json' : 'docs/mgui.min.json',
        MGUI_CUSTOM_JSON = 'mgui.custom.json',
        UNDEFINED = undefined,
    ...
    var loadData = function(url, callback, delayLoadRemote, waitRemote) {
        /*url链接，callback为回调函数，dealyLoadRemote为延迟加载的远程加载，waitRemote为等待远程*/
        var cacheData = null;
        ...
        if(isIndexJson && docIndex) {
            ...
            var storedData = $.mgui.store.get('//' + url, null);
            if(storedData !== null) {
                var storedVersion = $.mgui.store.get('//' + url + '::V');
                if(storedVersion) {
                    cacheData = {
                        data: storedData,
            ...
            $.ajax({
                ...
                success: function(data) {
                    if(data !== null) {
                        ...
                        $.mgui.store.set('//' + url, data);
                        $.mgui.store.set('//' + url + '::V', dataVersion);
    ...
    var displaySectionIcon = function($icon, section) {
        ...
        $icon.attr('class', 'fa').text('').css('background-image', '');
    ...
    var displaySection = function() {
        ...
        if(eachSection(function(chapter, section, $sectionList) {
                ...
                displaySectionIcon($head.children('.fa'), section);
    ...
    var chooseIcon = function($icon) {
        ...
        if(oldIcon) $preview.find('.fa').removeClass('fa-' + oldIcon);
        var icon = $icon.data('fa');
        ...
        $preview.find('.fa').addClass(id).toggleClass('fa-spin', isSpinner);
        $preview.find('.name').text(isSpinner ? ('fa-spin ' + id) : id);
    ...
    var queryIcon = function(keys) {
        ...
        var $section = $('#section-control-fa');
        $body.attr('data-query', 'icons');
        var $search = $section.children('.section-search');
        if(!$search.length) {
            $search = $('<div class="section-search card-content"><div class="loader loading"><i class="fa fa-spin fa-spinner"></i> 正在拼命加载中...</div></div>');
        ...
        loadData(ICONS_JSON, function(data) {
            ...
            if(!$list.length) {
                ...
                $.each(data, function(iconName, icon) {
                    var $li = $('<li id="control-icon-' + iconName + '" data-id="' + iconName + '"><a href="#control/icons/' + iconName + '"><i class="fa fa-' + iconName + '"></i> fa-' + iconName + '</a></li>');
                    icon.id = iconName;
                    $li.data('fa', icon).toggleClass('control-icon-shaped', !!icon.shaped);
                    $list.append($li);
                });
    ...
    var query = function(keyString) {
        ...
        $.each(keyString.split(' '), function(i, key) {
            ...
            } else if(key.startsWith('fa-') || key.startsWith('fa:')) {
                keyOption.type = 'fa';
                keyOption.val = key.substr(3);
            } else if(key.startsWith('i:') || key.startsWith('i-')) {
                keyOption.type = 'fa';
                keyOption.val = key.substr(0);
   ...
   /*展示页面的topic*/
    ...
    var handlePageLoad = function() {
        ...
        if(mguiPkg) {
            $pageBody.find('.mgui-version').text(mguiPkg.version);
        }
    ...
    var loadPage = function(section, topic, waitRemote) {
        ...
        loadData(section.url, function(data, dataType) {
            if(mguiPkg) data = data.format(mguiPkg, '{\\$0}');
    ...
    var openPage = function($section, section, topic) {
        ...
        displaySectionIcon($pageHeader.find('.fa'), section);
        ...
        $pageAttrs.children('.badge-source').toggle(!!section.url).attr('href', 'https://github.com/yangfeng002/module-MGUI/tree/master/' + section.url);
        var lib = section.lib;
        if(lib) {
            $pageAttrs.children('.badge-mgui').toggle(!!lib.bundles.standard);
            ...
            if(isShowCodeBadage) {
                ...
                var srcPrefix = 'https://github.com/yangfeng002/module-MGUI/tree/' + (mguiPkg.version ? ('v' + mguiPkg.version) : 'master') + '/';
                if(lib.srcCount) {
                    $dropdown.append('<li class="dropdown-header primary-header" data-type="source"><i class="fa fa-file-code"></i>  源码 ' + lib.srcCount + '</li>');
                ...
                if(lib.bundlesCount) {
                    $dropdown.append('<li class="dropdown-header primary-header" data-type="bundles"><i class="fa fa-cube"></i> 打包</li>');
                    if(lib.bundles.standard) {
                        $dropdown.append('<li class="dropdown-header">标准版</li>');
                        var files = [];
                        if(lib.src.js && lib.src.js.length) {
                            files.push('dist/js/mgui.js');
                        }
                        if(lib.src.less && lib.src.less.length) {
                            files.push('dist/css/mgui.css');
                        }
                    ...
                    if(lib.bundles.lite) {
                        ...
                        var files = [];
                        if(lib.src.js && lib.src.js.length) {
                            files.push('dist/js/mgui.lite.js');
                        }
                        if(lib.src.less && lib.src.less.length) {
                            files.push('dist/css/mgui.lite.css');
                        }
                        if(lib.src.resource && lib.src.resource.length) {
                            lib.src.resource.forEach(function(rf) {
                                if(rf.indexOf('//') > -1) {
                    ...
                    if(lib.code === 'theme') {
                        $dropdown.append('<li><a target="_blank" href="' + srcPrefix + 'dist/mgui-theme.css">dist/css/mgui-theme.css</a></li>');
                    }
                }
            }
        }
        ...
        toggleCompactMode(true, function() {
            ...
            setTimeout(function() {
                ...
                setTimeout(function() {
                    ...
                    if(firstOpenPage) {
                        firstOpenPage = $.mgui.store.get('first_open_page', true);
                        if(firstOpenPage) {
                            firstOpenPage = false;
                            $.mgui.store.set('first_open_page', false);
                            if(!isTouchScreen) {
                                setTimeout(function() {
    ...
    var openSection = function(section, topic) {
        ...
        if(section.url === '') {
            $.mgui.messager.show('该链接所指示的文档尚未完成。你可以Fork项目来完善文档。');
            return;
        }
        ...
        switch(section.target) {
            case 'external':
                ...
                break;
            case 'page':
                var pageViewLayout = $.mgui.store.get('pageViewLayout');
                if(!pageViewLayout && $(window).width() >= 1600) {
                    $('#changeViewModal').on('hide.mgui.modal', function() {
                        pageViewLayout = $.mgui.store.get('pageViewLayout');
                        if(!pageViewLayout) $.mgui.store.set('pageViewLayout', 'single');
            ...
            $.each(build.bundles, function(idx, name) {
                ...
                if(!bundleBuild && buildLib) {
                    bundleBuild = {
                        title: buildLib.name,
                        dest: 'dist/lib/' + name + '/',
                        filename: (buildLib.source && buildLib.source !== 'Bootstrap') ? name : ('mgui.' + name),
                        includes: [name],
    ...
    var loadPackage = function(callback) {
        loadData(PKG_JSON, function(pkg) {
            loadData(MGUI_JSON, function(mgui) {
                // loadData(MGUI_CUSTOM_JSON, function(customMGui) {
                    mguiPkg = $.extend(pkg, {
                        lib: $.extend({}, mgui.lib/*, customMGui ? customMGui.lib : null*/),
                        builds: $.extend({}, mgui.builds/*, customMGui ? customMGui.builds : null*/)
                    });
                    if($.doc) $.doc.pkg = mguiPkg;
                    callback(mguiPkg);
                // }, null, true);
            }, null, true);
        }, null, true);
            ...
            $.each(data.lib, function(itemName, item) {
                ...
                $.each(pkgLibs, function(idx, sLib) {
                    ...
                    if(sLib.indexOf(itemName) > -1) {
                        $td.addClass('success').html('<i class="text-success fa-check"></i>');
                    } else {
                        $td.html('<i class="text-muted fa-remove"></i>');
                    }
                    $tr.append($td);
                });
                ...
                if(item.source) {
                    ...
                    isMGUI = true;
                    $td.append('<span data-toggle="tooltip" title="License: MIT">MGUI</span>');
                }
                $tr.append($td);

                $tr.append('<td>' + (item.source === 'Bootstrap' ? 'MIT' : (isMGUI ? 'MIT' : (item.license || ''))) + '</td>');
    ...
    var initCopyable = function() {
        if(!$.mgui.browser.isIE() || $.mgui.browser.ie > 8) {
            ...
            $copyCodeBtn.on('hide.mgui.tooltip', function() {
            ...
            $(document).on('mouseenter', 'pre.prettyprint, .copyable', function() {
                ...
                if(!$copyableTarget.attr('id')) {
                    $copyableTarget.attr('id', 'code-' + $.mgui.uuid())
                }
    ...
    var changeTheme = function(theme, callback) {
        ...
        var readyChangeTheme = function(css) {
            ...
            if(css) $.mgui.store.set('doc_theme', theme);
            else $.mgui.store.remove('doc_theme');
        };
    ...
    var initTheme = function() {
        ...
        var savedTheme = $.mgui.store.get('doc_theme');
        if(savedTheme) {
    ...
    var initChangeView = function() {
        var changePageView = function(pageViewLayout, notSave) {
            if(pageViewLayout) {
                if(!notSave) $.mgui.store.set('pageViewLayout', pageViewLayout);
            } else {
                pageViewLayout = $.mgui.store.get('pageViewLayout');
            }
        ...
        $modal.on('show.mgui.modal', function() {
    ...
    var init = function() {
        ...
        // check storage
        storageEnable = $.mgui.store && $.mgui.store.enable;
        ...
        $grid.on('click', '.card-heading', function(e) {
            ...
            stopPropagation(e);
        }).on('click', '.card-heading > h5 > .name, .card-heading > .fa', function(e) {
            ...
            e.preventDefault();
            stopPropagation(e);
        }).on('mouseenter', '.card-heading > h5 > .name, .card-heading > .fa', function() {
            $(this).closest('.card-heading').addClass('hover');
        }).on('mouseleave', '.card-heading > h5 > .name, .card-heading > .fa', function() {
            $(this).closest('.card-heading').removeClass('hover');
        }).on('mouseenter', '#section-control-fa .section-search > ul > li > a', function() {
            oldActivePreivewId = $('#section-control-icon').data('preview');
            chooseIcon($(this).closest('li'));
        }).on('mouseleave', '#section-control-fa .section-search > ul > li > a', function() {
            if(oldActivePreivewId) {
                chooseIcon($('#control-icon-' + oldActivePreivewId));
            }
        }).on('click', '#section-control-fa .section-search > ul > li > a', function() {
