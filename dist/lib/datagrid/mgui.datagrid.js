...
(function($, undefined) {
    ...
    var DataGrid = function(element, options) {
        ...
        that.name      = NAME;
        that.uuid      = $.mgui.uuid();
        that.id        = 'mgui-datagrid-' + that.uuid;
        ...
        if (options.checkable) {
            if (options.selectable && $.fn.selectable) {
                ...
                }, $.isPlainObject(options.selectable) ? options.selectable : null)).data('mgui.selectable');
    ...
    DataGrid.prototype.renderCell = function(rowIndex, colIndex, $row) {
        ...
        if (colIndex > 0 && rowIndex === 0 && options.sortable && config.sort !== false) {
            ...
            if (!$sorter.length) {
                $sorter = $('<div class="datagrid-sorter"><i class="fa fa-sort"></i></div>').appendTo($cell);
