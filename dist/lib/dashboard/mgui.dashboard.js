...
(function($, Math, undefined) {
    ...
    var dashboardMessager = $.mgui.Messager ? new $.mgui.Messager({placement: 'top', time: 1500, close: 0, scale: false, fade: false}) : 0;
    ...
    Dashboard.prototype.updatePanelHeight = function() {
        ...
        var sizeConfig = {};
        if(that.id && $.mgui.store) {
            sizeConfig = $.mgui.store.pageGet('mgui.dashboard.' + that.id + '.sizeConfig', sizeConfig);
        }
        ...
        if(that.id && $.mgui.store) {
            $.mgui.store.pageSet('mgui.dashboard.' + that.id + '.sizeConfig', sizeConfig);
        }
    ...
    Dashboard.prototype.init = function() {
        ...
        if(options.data) {
            ...
            $.each(options.data, function(idx, config) {
                ...
                var $panel = $('<div class="panel" data-id="' + (config.id || $.mgui.uuid()) + '"/>');
    ...
    $.fn.dashboard = function(option) {
        return this.each(function() {
            var $this = $(this);
            var data = $this.data('mgui.dashboard');
            var options = typeof option == 'object' && option;

            if(!data) $this.data('mgui.dashboard', (data = new Dashboard(this, options)));
